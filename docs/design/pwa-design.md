...
- アイコン: 新ロゴ「Readout」(オーナー採用決定済み)を使用する。L字軸+黄金分割の
  目盛+昇るデータ点2つ+星で構成し、星から両軸へ点線(座標の投影)を落とした、
  「グラフから値を読み取る」動作そのものの図案。点・星のサイズ比は 1:φ:φ²、
  間隔は 1:φ、単色 #1565C0(theme_colorと同一)。マスターは `public/logo.svg`
  (青マーク・透過)で、`public/pwa-192.png` / `public/pwa-512.png`(青地・白マーク)と
  `public/favicon.ico`(角丸青地・白マーク、16/32/48px、小サイズ用に線を約1.5倍に
  太らせた別描画)はここから生成した
