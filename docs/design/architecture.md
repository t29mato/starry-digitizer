...
読者: 本リポジトリに新しく入る開発者、および組み込み側(Starrydata2 の新 UI)の開発者

> **組み込み側の呼称について。** 2026-09-11 に「Starrydata3」という呼び方は廃止され、
> 製品名は **Starrydata2**、システムのバージョンが **v4**、画面は **新 UI**(従来のものは
> 旧 UI)と呼ぶことになった。3 代目の別製品ではなく、同じ製品の次のバージョンで、DB も
> starrydata2 のまま 2 つの UI が同居する、という整理による(`v3` は既に使用済み。
> GitHub に `starrydata3` という同名の別プロジェクトが実在することも分かっている)。
> 本ドキュメントより前の日付を持つ設計メモ(`framework-dependency-review.md` 2026-09-03、
> `engine-boundary.md` 2026-09-04)は**当時の呼称のまま**にしてある。当時の判断を当時の
> 言葉で残すためで、誤りではない。**一括置換をしないこと。**
