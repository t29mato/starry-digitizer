...
.. warning::
   ...
   **既にあるラッパを外すときは、そこに書いた** ``--sd-*`` **の上書きも一緒に
   移してください。** ラッパは「CSS を効かせるスコープ」と「``--sd-primary`` などを
   設定する場所」の 2 役を兼ねていました。今回無くなったのは前者だけです。
   カスタムプロパティは継承で届くので、``.starry-digitizer { --sd-*: … }`` と
   書いていた上書きは、パネルがラッパの外に出た瞬間に届かなくなります。
   **エラーは出ません**\ ——パネルは既定のパレットで正常に動きます。
   上書きは ``.sd-panel``\ 、共通の祖先要素、または ``:root`` に移してください。
   実際にこれを踏んだホストは、3 サイズのスクリーンショット比較でしか
   気づけませんでした。
...
- **1 つの context に canvas は 1 セット、つまり** ``CanvasMain`` **は 1 つだけです。**
  同じ context に 2 つ目を置くと後からマウントしたほうが slot を取り、先に置いた
  ほうは描画を続けているつもりで実際にはどこにも描かなくなります。
  ``attachCanvases()`` は既に付いている要素を **別の要素で** 置き換えるときに
  warn しますが、気づける手がかりはそれだけです。
  **返すときはキー名ではなく要素を渡してください。**
  ``detachCanvases({ wrapper, imageCanvas, maskCanvas, tempMaskCanvas })``
  のように貸した本人が渡すと、まだその要素が入っている slot だけが返ります。
  2 つ目のコンポーネントのアンマウントが 1 つ目の canvas を巻き添えに外す、
  という事故が起きません(``v-if`` でキャンバス列を畳むと踏みます)。
  キー配列版と引数なし版は無条件に外すので、全体の teardown 専用と考えてください。
...
- **マーカーの寸法は「画面の px」ではなく「倍率」に属します。** オーバーレイの座標は
  画像ピクセル × ``canvasHandler.scale`` なので、寸法を固定 px にすると、縮小するほど
  マーカーが **図に対して** 大きくなります。16% では 10px の点が元画像の 62.5px 相当を
  覆い、点が数十個ある図は自分のマーカーで曲線が読めなくなります。
  ``scaledMarkerSizePx(basePx, canvasHandler.scale, minPx, maxPx)`` と ``STYLE`` の
  上下限(``POINT_MIN_SIZE_PX`` / ``POINT_MAX_SIZE_PX``、一時点と軸にも同じ対)を
  使ってください。上下限は必須です。単純な ``basePx * scale`` は 16% で 1.6px になり、
  見えないうえに掴めません。
- **当たり判定は見た目より大きく保ってください。** ``STYLE.POINT_HIT_MIN_SIZE_PX`` が、
  組み込みの ``CanvasPoint`` が(透明な)当たり判定に与える下限です。点がどれだけ
  小さく描かれても掴めます。
- **判定領域が重なったときは、z 順ではなく距離で決めてください。** 下限がある以上、
  点がそれより近い場所ではマーカーが必ず重なります。ブラウザはクリックを
  **上にある要素** に渡しますが、それはユーザーが狙った点とは限りません。
  実データでの計測では、点の間隔 6〜12px の帯で **クリックの約 1/3 が別の点に入り**\ 、
  そのうち半分は **最近傍ですらありません** でした(ある点の中心をちょうど押したのに
  8.06px 離れた点が取った例があります)。``nearestPointId(points, coord, hitSizePx)``
  ——core から export しており、``CanvasPoint`` もこれを使います——が、クリックを覆う
  マーカーのうち最も近いものを選びます(同距離なら上にあるほう)。
  おおむね 40% を下回る倍率ではどんな規則でも救えません。そこでは点の間隔が 1px 未満
  なので、密な図の編集は拡大してから行うものです。低倍率は「打ち漏らしを探す」ための
  もので、上の寸法の規則が守っているのはそちらの用途です。
