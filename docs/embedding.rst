...
ルールはすべて ``.starry-digitizer`` 配下にスコープされ、ホストのグローバル CSS と
...
自前配置ではパネルを ``.starry-digitizer`` クラスの要素で包む必要があります。
どこに置くかで見た目が変わるので、10.7 を参照してください。
...
10.7 ``.starry-digitizer`` ラッパをどこに置くか
--------------------------------------------------
...
``font-size: 16px``\ 、``line-height`` は未指定)。

高さは ``--sd-axis-list-min-height`` / ``--sd-dataset-list-min-height`` の既定が ``vh``
なので、ビューポートの高さが変われば 3 通りとも一緒に動きます。A と B の差だけが意味を
持つ数字です。なお測定機には DejaVu 系のフォントしか入っておらず、``system-ui`` も
``Roboto`` も同じ実フォントに落ちます。表の ``font-family`` は ``getComputedStyle`` が
返す指定値で、A と B の高さの差は書体の違いではなくサイズ・行送り・ユーティリティの差です。
...
.. list-table::
   ...
   * - パネルの基準フォント(14px / Roboto を指定)
     ...
     - **16px / system-ui のまま**
     - 14px / Roboto
   * - パネルの ``line-height``\ (1.4 を指定)
     - ``19.6px``
     - **normal のまま**
     - ``19.6px``
   * - パネルの ``color`` / ``box-sizing``
     - ``rgba(0, 0, 0, 0.87)`` / ``border-box``
     - **初期値のまま**\ (``rgb(0, 0, 0)`` / ``content-box``)
     - ``rgba(0, 0, 0, 0.87)`` / ``border-box``
   * - パネルの ``<h4>``\ (トークン未指定)
     - ``17.6px`` / 600 / margin ``10px 0 4px``
     - **ブラウザ既定のまま**\ (``16px`` / 700 / margin ``21.28px 0``)
     - ``17.6px`` / 600 / margin ``10px 0 4px``
   * - ``.d-flex`` / ``.pa-1`` / ``.mb-2``
     - ``flex`` / 4px / 8px
     - **効かない**\ (``block`` / 0 / 0)
...
**推奨は A** です。ライブラリのパネルだけがライブラリのユーティリティと ``h4`` / ``h5``
既定を受け取り、ホストの UI は自分のタイポグラフィと自分のクラス定義のままになります。
テーマの上書き(``.starry-digitizer { --sd-primary: … }``)も、インスタンスごとの
レイアウト変数(10.6 の ``--sd-height`` など)も、この要素にまとめて書けます。ラッパは
1 ページに何個あっても構いません。パネルを離れた場所に置くなら、その領域ごとにラッパを
付けてください。

**C の落とし穴**\ : ラッパの中に入れたホストの UI にも、ライブラリの ``h4`` / ``h5``
既定とユーティリティが当たります。上の実測では、トークンを何も指定していなくても、
ホスト側の ``<h4>`` の margin が ``21.28px 0`` から ``10px 0 4px`` に(``font-size`` も
16px から 17.6px に)、ホスト側にたまたま存在した ``.d-flex`` が ``block`` から ``flex``
に変わりました。ホストが自分のクラス名に ``d-flex`` / ``pa-1`` など同名のユーティリティを
使っていると、ラッパの中では意図しない側が当たります。

書体については、タイポグラフィのトークンが ``inherit`` 既定になったことで
**何も指定しなければ化けなくなりました**\ 。上の実測でも、トークン未指定の C では
ホスト領域は 16px / system-ui のままです。ただし
``.starry-digitizer { --sd-font-size: 14px }`` のように指定すると、その宣言はラッパ配下
すべてに効くので、C ではホストの UI も一緒にそのサイズになります(実測では 16px /
system-ui から 14px / Roboto に化けました)。書体を指定したいなら A で分離してください。

**B の落とし穴**\ : **色も書体もそのまま出るので、パッと見は動いているように見えます**\ 。
テーマトークンは ``:where(:root)`` にあるため、プライマリ色のボタンは A と同じ
``rgb(30, 136, 229)`` で塗られ、ホストが ``:root`` に書いた上書きもそのまま効きます。
タイポグラフィの既定が ``inherit`` になった今は、フォントもサイズも行送りも A と B で
完全に一致します(どちらもホストの地の文をそのまま継ぐため)。
**以前あった「B だけ 16px / system-ui に落ちる」という分かりやすい兆候は無くなりました**\ 。

代わりに壊れるのは、ラッパ配下にしかないもの、すなわちユーティリティクラス・``h4`` /
``h5`` の既定・``color`` / ``box-sizing``\ 、そして書体トークンを指定した場合のその適用です。
影響はパネルによって大きく違い、トークン未指定の上の条件では ``AxisSetManager`` が
121.0px → 138.3px、\ ``DatasetManager`` が 205.0px → 222.3px と **1 割前後しか伸びません**\ 。
「少し余白が違う」ようにしか見えず、見落とします。一方 flex 行を多く持つ
``ExtractorSettings`` は 300.0px → 558.8px と **9 割近く伸びて明確に崩れます**\ 。
パネルによって崩れ方が桁違いなので、1 枚だけ見て確かめると気付けません。
なお ``.starry-digitizer { --sd-primary: … }`` という文書化済みのテーマ上書きは、
B では付ける要素が無いため **黙って無視されます**\ (``:root`` に書いた上書きは効くので、
...
上のレシピは「ユーザーの 1 操作 = ``'capture'`` 1 回」に依存しているので、
デジタイザ側が capture する操作を全て挙げます。
...
.. list-table::
   ...
   * - データセットの追加 / 削除 / 全削除 / 中身のクリア
     - それぞれ 1
   * - データセットの切り替え、View All、ズーム・モード・マスクの変更、補間の ON/OFF
     - 0(取り消せるデータは変わらない)
...
- Undo が戻すのはプロットと軸であって**選択状態ではありません**。``⌘Z`` の直後は
  何も選択されていないので、次の矢印キーは何も動かしません。
