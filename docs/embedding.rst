...
- **当たり判定は見た目より大きく保ってください。** ``STYLE.POINT_HIT_MIN_SIZE_PX`` が、
  ...
  小さく描かれても掴めます。ごく低い倍率では判定領域どうしが必然的に重なりますが
  (点が画面上で数 px しか離れていないため)、その場合は **上に描かれたマーカー** が
  クリックを取ります。ユーザーが上に見ているものと一致します。密な図の編集は
  拡大してから行うもので、低倍率は「打ち漏らしを探す」ためのものです。上の寸法の
  規則が守っているのはそちらの用途です。
