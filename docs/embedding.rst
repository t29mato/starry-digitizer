...
実測値(Chromium 実機。``AxisSetManager`` / ``ExtractorSettings`` / ``DatasetManager`` を
...
**推奨は A** です。ライブラリのパネルだけがライブラリの基準フォントとユーティリティを受け取り、
ホストの UI は自分のタイポグラフィのままになります。テーマの上書き
(``.starry-digitizer { --sd-primary: … }``)も、インスタンスごとのレイアウト変数
