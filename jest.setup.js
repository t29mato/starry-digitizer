...
// Mock CanvasRenderingContext2D as a class if not natively defined, so that
// `context instanceof CanvasRenderingContext2D` checks (e.g. HTMLCanvas.ts)
// pass under plain jsdom (no `canvas` package installed).
if (typeof global.CanvasRenderingContext2D === 'undefined') {
  global.CanvasRenderingContext2D = class CanvasRenderingContext2D {}
}

// Cache one mock context per canvas element, since callers may read
// `getContext('2d')` repeatedly and expect the same object back (e.g. to
// assert on the same jest.fn() spy across multiple calls).
const mockContextsByCanvas = new WeakMap()
    ...
    value: function (contextType) {
      if (contextType === '2d') {
        if (canvasModule) {
          ...
          if (mockContextsByCanvas.has(this)) {
            return mockContextsByCanvas.get(this)
          }
          // Provide a basic mock context
          const mockContext = Object.assign(
            Object.create(global.CanvasRenderingContext2D.prototype),
            {
              canvas: this,
              fillStyle: '',
              strokeStyle: '',
          ...
          mockContextsByCanvas.set(this, mockContext)
          return mockContext
        }
      }
      return null
...
if (typeof HTMLCanvasElement !== 'undefined') {
  console.log(
    '✅ Jest canvas setup complete - HTMLCanvasElement.getContext and Image are now available (jsdom)',
  )
} else {
