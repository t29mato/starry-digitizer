...
export interface UnifiedAxisExtractionOptions {
  ...
  useColorThreshold?: boolean // Whether to use color threshold or not (default: true)
  minAreaRatio?: number // Minimum area ratio for plot area detection (default: 0.5 = 50%)
  minSolidity?: number // Minimum solidity for rectangle detection (default: 0.9 = 90%)
}
...
export class UnifiedAxisExtractor implements AxisExtractorInterface {
  ...
  setUseColorThreshold(useColorThreshold: boolean): void {
    this.options.useColorThreshold = useColorThreshold
    // If adapter is BrowserAdapter, update its use color threshold setting
    if (this.adapter instanceof BrowserAdapter) {
      ;(this.adapter as any).options = { ...this.options }
    }
  }
  ...
  setMinSolidity(solidity: number): void {
    this.options.minSolidity = solidity
    // If adapter is BrowserAdapter, update its min solidity
    if (this.adapter instanceof BrowserAdapter) {
      ;(this.adapter as any).options = { ...this.options }
    }
  }
  ...
  private getImageWidth(imageSource: any): number {
    ...
    if (imageSource.canvas?.width !== undefined) return imageSource.canvas.width
    return 800 // デフォルト値
  }
